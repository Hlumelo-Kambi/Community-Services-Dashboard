#!/usr/bin/env python3
"""
Municipal Complaints Database CLI with Plotext Visualizations
A command-line interface for querying municipal complaints data with visual charts
"""
...
class ComplaintsDatabase:
    ...
    def print_results(self, results: List[sqlite3.Row], title: str = "Query Results"):
        ...
        # Print rows (limit to first 10 for better readability when showing charts)
...
class ComplaintsCLI:
    ...
    def create_visualization(self, x_data, y_data, title, x_label, y_label, viz_type):
        """Create visualization based on user preference"""
        if viz_type == '4':  # No visualization
            return
            
        plt.clear_data()
        
        if viz_type == '1':  # Vertical bar
            plt.bar(x_data, y_data)
            plt.xlabel(x_label)
            plt.ylabel(y_label)
        elif viz_type == '2':  # Horizontal bar
            plt.bar(y_data, x_data, orientation="horizontal")
            plt.xlabel(y_label)
            plt.ylabel(x_label)
        elif viz_type == '3':  # Histogram
            plt.hist(y_data, bins=min(10, len(y_data)))
            plt.xlabel(y_label)
    ...
    def query_1_active_complaints(self):
        """View all active complaints with visualization"""
        query = '''
            SELECT 
                c.complaint_id,
        ...
        if results:
            ...
            viz_type = self.get_visualization_preference()
            if viz_type == '4':  # No visualization
                return
                
            # Visualization: Active complaints by category
    ...
    def query_2_complaints_by_category(self):
        """View complaints by category with visualization"""
        ...
        try:
            ...
            if results:
                ...
                # Visualization: Status distribution for this category
                ...
                self.create_visualization(
                    ...
                    f"Status Distribution for {cat_name}",
                    "Status",
                    "Number of Complaints",
                    viz_type
                )
    ...
    def query_3_complaints_by_ward(self):
        """View complaints by ward with visualization"""
        try:
            # Show all available wards first
            print("\nAvailable wards:")
            ward_query = "SELECT DISTINCT ward FROM residents ORDER BY ward"
            wards = self.db.execute_query(ward_query)
            for w in wards:
                print(f"Ward {w['ward']}")

            # Ask user to pick one
            ward = input("\nEnter ward number: ").strip()
            ...
            if results:
                ...
                # Visualization: Complaints by category in this ward
                categories = [row['category_name'] for row in results]
                category_counts = Counter(categories)
                ...
                self.create_visualization(
                    list(category_counts.keys()), 
                    list(category_counts.values()),
                    f"Complaints by Category in Ward {ward}",
                    "Category",
                    "Number of Complaints",
                    viz_type
                )
    ...
    def query_4_resident_history(self):
        """View resident complaint history with visualization"""
        try:
            # Show all available residents
            print("\nAvailable residents:")
            res_query = "SELECT resident_id, first_name || ' ' || last_name as name FROM residents ORDER BY resident_id"
            residents = self.db.execute_query(res_query)
            for res in residents:
                print(f"{res['resident_id']}: {res['name']}")

            # Ask user to pick one
            resident_id = input("\nEnter resident ID: ").strip()
            ...
            if results:
                ...
                # Visualization: Timeline of complaints
                dates = [row['submission_date'] for row in results]
                date_counts = Counter(dates)
                
                self.create_visualization(
                    list(date_counts.keys()), 
                    list(date_counts.values()),
                    f"Complaint Timeline for {resident_name}",
                    "Date",
                    "Number of Complaints",
                    viz_type
                )
    ...
    def query_5_resolution_statistics(self):
        """View complaint resolution statistics with visualization"""
        query = '''
            SELECT 
                sc.category_name,
        ...
        if results:
            ...
            # Visualization: Resolution rates by category
            categories = [row['category_name'] for row in results]
            resolution_rates = [float(row['resolution_rate']) for row in results]
    ...
    def query_6_overdue_complaints(self):
        """View overdue complaints with visualization"""
        query = '''
            SELECT 
                c.complaint_id,
                r.first_name || ' ' || r.last_name as resident_name,
        ...
        if results:
            ...
            # Visualization: Days overdue histogram
            days_old = [int(row['days_old']) for row in results]
            
            self.create_visualization(
                list(range(len(days_old))), 
                days_old,
                "Distribution of Overdue Complaints by Days",
                "Complaint Index",
                "Days Overdue",
                viz_type
            )
    ...
    def query_7_complaints_by_status(self):
        """View complaints by status with visualization"""
        ...
        if results:
            ...
            # Visualization: Category breakdown for this status
            categories = [row['category_name'] for row in results]
            category_counts = Counter(categories)
            ...
            self.create_visualization(
                list(category_counts.keys()), 
                list(category_counts.values()),
                f"Category Breakdown for {status} Complaints",
                "Category",
                "Number of Complaints",
                viz_type
            )
    ...
    def query_8_top_complaint_categories(self):
        """View top complaint categories with visualization"""
        query = '''
            SELECT 
                sc.category_name,
        ...
        if results:
            ...
            # Visualization: Horizontal bar chart of complaint categories
            categories = [row['category_name'] for row in results]
            percentages = [float(row['percentage']) for row in results]
    ...
    def query_9_ward_performance(self):
        """View ward performance summary with visualization"""
        query = '''
            SELECT 
                r.ward,
                COUNT(*) as total_complaints,
        ...
        if results:
            ...
            # Visualization: Ward performance comparison
            wards = [f"Ward {row['ward']}" for row in results]
            total_complaints = [int(row['total_complaints']) for row in results]
            resolution_rates = [float(row['resolution_rate']) for row in results]
    ...
    def query_10_complaint_timeline(self):
        """View complaint timeline for specific complaint with visualization"""
        try:
            ...
            if timeline:
                ...
                dates = [row['status_date'] for row in timeline]
                
                # Create a simple timeline visualization
                status_nums = {status: i for i, status in enumerate(set(statuses))}
                y_values = [status_nums[status] for status in statuses]
                x_values = list(range(len(timeline)))
                
                self.create_visualization(
                    x_values, 
                    y_values,
                    f"Status Timeline for Complaint {complaint_id}",
                    "Timeline Sequence",
                    "Status Level",
                    viz_type
                )
    ...
    def run(self):
        """Main CLI loop"""
        print("Welcome to the Municipal Complaints Database CLI with Visualizations!")
